...
namespace SharpUtility.InputSimulate
{
    ...
    public static class WindowList
    {
        ...
        public static List<WindowInformation> GetAllWindows()
        {
            ...
            var winInfo = new List<WindowInformation> {WinInfoGet(desktopWindow)};
            var handles = GetChildWindows(desktopWindow);
        ...
        {
            var desktopWindow = WinInfoGet(GetDesktopWindow());
        ...
        ///     A handle to the parent window whose child windows are to be searched. If hwndParent is NULL,
        ///     the function uses the desktop window as the parent window. The function searches among windows that are child
        ///     windows of the desktop. If hwndParent is HWND_MESSAGE, the function searches all message-only windows.
        /// </param>
        /// <param name="hwndChildAfter">
        ///     A handle to a child window. The search begins with the next child window in the Z order.
        ///     The child window must be a direct child window of hwndParent, not just a descendant window. If hwndChildAfter is
        ///     NULL, the search begins with the first child window of hwndParent. Note that if both hwndParent and hwndChildAfter
        ///     are NULL, the function searches all top-level and message-only windows.
        /// </param>
        /// <param name="lpszClass">
        ///     Optional: The class name or a class atom created by a previous call to the RegisterClass or
        ///     RegisterClassEx function. The atom must be placed in the low-order word of lpszClass; the high-order word must be
        ///     zero. If lpszClass is a string, it specifies the window class name. The class name can be any name registered with
        ...
        ///     Optional: The window name (the window's title). If this parameter is NULL, all window names
        ///     match.
        /// </param>
        /// <returns></returns>
        ...
        ///     terminating null character. If the function fails, the return value is zero. To get extended error information,
        ///     call GetLastError.
        /// </returns>
        ...
        private static List<WindowInformation> GetChildWindowsInfo(WindowInformation parent)
        {
            ...
            var childHwnd = GetWindow(parent.Handle, GetWindow_Cmd.GW_CHILD);
            while (childHwnd != IntPtr.Zero)
            {
                var child = WinInfoGet(childHwnd);
                child.Parent = parent;
        ...
        {
            var winInfoList = new List<WindowInformation> {winInfo};
            foreach (var child in winInfo.ChildWindows)
        ...
        private static WindowInformation WinInfoGet(IntPtr hWnd)
        {
            ...
            else
            {
                caption =
                    new StringBuilder(
                        Convert.ToInt32(SendMessage(wi.Handle, WMConstants.WM_GETTEXTLENGTH, IntPtr.Zero, IntPtr.Zero)) +
                        1);
